...
test('BenchmarkMetricsCollector - records metrics accurately', () => {
  ...
  collector.recordCall({ name: 'chrome_navigate', args: { url: 'http://localhost/test' }, durationMs: 120, isError: false });
  collector.recordCall({ name: 'chrome_read_page', args: {}, durationMs: 80, isError: false });
  collector.recordCall({ name: 'chrome_read_page', args: { filter: 'interactive' }, durationMs: 60, isError: false });
  collector.recordCall({ name: 'chrome_javascript', args: { code: 'document.title' }, durationMs: 45, isError: false });
...
test('Mock Runner - runs scenarios deterministically with mock transport', async () => {
  ...
  const mockHandler = {
    async handleToolCall(name, args) {
      ...
      if (name === 'chrome_read_page') {
        ...
        if (mockState.dynamicSelected) content += ' select-btn-us-east-prod US-East Production Primary ALLOC-8891 Allocation confirmed';
        else content += ' select-btn-us-east-prod US-East Production Primary';
        if (mockState.siteBAuthorized) content += ' DISPATCHED-OK-2026 Dispatched successfully';
      ...
      if (name === 'chrome_click_element') {
        ...
        if (args.selector === '#select-btn-us-east-prod' || args.selector === '#submit-allocation-btn') mockState.dynamicSelected = true;
        if (args.selector === '#authorize-dispatch-btn') mockState.siteBAuthorized = true;
        return { success: true };
      }
      if (name === 'chrome_fill_or_select') return { success: true };
      if (name === 'chrome_javascript') return { token: 'BENCHMARK-CODE-9204', shipmentId: 'SHP-88301' };
      return { success: true };
    },
  };
  ...
  col1.start();
  const sum1 = await runShortScenario(client, col1, { fixtureBaseUrl: 'http://localhost:12399' });
  ...
  col2.start();
  const sum2 = await runDynamicScenario(client, col2, { fixtureBaseUrl: 'http://localhost:12399' });
  ...
  col3.start();
  const sum3 = await runMultiHopScenario(client, col3, { fixtureBaseUrl: 'http://localhost:12399' });
...
test('BenchmarkMetricsCollector - tracks multi-tool inspections and DOM mutation resets', () => {
  ...
  collector.recordCall({ name: 'chrome_javascript', args: { code: 'document.querySelector("#btn").textContent' }, isInspection: true });
...
test('BenchmarkMetricsCollector - autoClassifyJsQueries classifies DOM inspection queries', () => {
  const collector = new BenchmarkMetricsCollector('auto-classify-test', { autoClassifyJsQueries: true });
  collector.start();
  collector.recordCall({ name: 'chrome_navigate', args: { url: 'http://localhost/test' } });
  collector.recordCall({ name: 'chrome_javascript', args: { code: 'document.querySelector(".badge").innerText' } });
  collector.recordCall({ name: 'chrome_javascript', args: { code: 'document.getElementById("status").textContent' } });
  const summary = collector.finish(true);
  assert.equal(summary.pageInspectionCalls, 2);
  assert.equal(summary.repeatedPageInspections, 1);
...
test('BenchmarkMetricsCollector - long JS script mutations (>50 chars) properly reset inspection state', () => {
  const collector = new BenchmarkMetricsCollector('long-script-mutation-test', { autoClassifyJsQueries: true });
  collector.start();
  collector.recordCall({ name: 'chrome_read_page' });
  ...
  const summary = collector.finish(true);
  assert.equal(summary.pageInspectionCalls, 2);
  assert.equal(summary.repeatedPageInspections, 0, 'Inspection after long JS mutation must not be repeated');
});

test('BenchmarkMetricsCollector - JS value setters are treated as mutations, not queries', () => {
  const collector = new BenchmarkMetricsCollector('setter-test', { autoClassifyJsQueries: true });
  collector.start();
  collector.recordCall({ name: 'chrome_read_page' });
...
test('BenchmarkMetricsCollector - classifies chrome_computer actions appropriately', () => {
  const collector = new BenchmarkMetricsCollector('computer-test');
  collector.start();
  const c1 = collector.recordCall({ name: 'chrome_computer', args: { action: 'screenshot' } });
  assert.equal(c1.isInspection, true);
  assert.equal(c1.isMutation, false);
  const c2 = collector.recordCall({ name: 'chrome_computer', args: { action: 'click', coordinate: [100, 200] } });
  assert.equal(c2.isInspection, false);
  assert.equal(c2.isMutation, true);
  const summary = collector.finish(true);
  assert.equal(summary.pageInspectionCalls, 1);
  assert.equal(summary.repeatedPageInspections, 0);
});
